...
def func():
    ...
    # n = int(input('Input uneven number\n'))
    global repeats
    repeats += 1
    ng = int(spiralsize.get())
    n = ng if ng % 2 else ng - 1
    print(n)
    num = np.power(n, 2)
    ...
    for i in range(np.add(center, 1)):
        ...
        for j in range(2 * i + 1):
            if not j:
                continue
            array[np.add(center, i)][np.subtract(np.add(center, i), j)] = np.subtract(np.add(np.multiply(8, i), prev),
                                                                                      j)
    ...
    label1.image = test
    label1.grid(row=2, column=3+repeats)
...
tkinter.Label(root, text='Spiral size').grid(row=0, column=1)
spiralsize = tkinter.Entry(root)
spiralsize.grid(row=0, column=2)
btn = tkinter.Button(root, command=func, text='Go!').grid(row=1, column=1)
root.mainloop()
